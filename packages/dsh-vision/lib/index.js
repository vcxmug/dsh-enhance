...
export function apply(ctx, config = {}) {
    ...
    async function callVision(imageUrl, question, detail, signal) {
        const cfg = scope.get();
        if (!cfg.baseUrl)
            throw new Error('vision: baseUrl not configured — set it in Settings → Plugins → dsh-vision');
        if (!cfg.model)
            throw new Error('vision: model not configured — set it in Settings → Plugins → dsh-vision');
    ...
    const describeTool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!image)
                throw new Error('vision_describe: image is required');
