...
/** Shape of a Responses API payload we read from (the fields we consume). */
...
/** Shape of a models-list payload we read from. */
...
const NAMESPACE = 'vision' as unknown as SettingsNamespace
...
export function apply(ctx: Context, config: Partial<VisionConfig> = {}) {
  ...
  async function toImageUrl(image: string, signal: AbortSignal | undefined): Promise<string> {
    const cfg = scope.get()
    ...
    try {
      ...
      throw new Error(`vision: cannot read image ${image}: ${error instanceof Error ? error.message : String(error)}`)
    }
  ...
  ): Promise<{ answer: string; model: string; status: string }> {
    const cfg = scope.get()
  ...
  const describeTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      const detail = args.detail ?? cfg.detail
  ...
  const listTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        let json: ModelsPayload
        try {
          json = await response.json() as ModelsPayload
        } catch {
          throw new Error(`vision: endpoint returned non-JSON (HTTP ${response.status}): ${String(response.statusText || '').slice(0, 100)}`)
        }
        if (!response.ok || json.error) {
