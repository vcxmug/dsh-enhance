...
export function apply(ctx, config = {}) {
    ...
    async function baseUrl() {
        ...
        throw new Error(`native-web: no local web instance detected at ${candidates.join(', ')} — start it first (docker compose up -d)`);
    }
    ...
    const searchTool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!query)
                throw new Error('native_search: query is required');
    ...
    const scrapeTool = defineTool({
        ...
        async execute(args, exec) {
            ...
            if (!url)
                throw new Error('native_scrape: url is required');
