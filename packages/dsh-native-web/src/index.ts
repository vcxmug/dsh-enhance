...
/** One ranked search result as returned to the model. */
interface SearchResult {
  title: string
  url: string
  description: string
  /** The output schema allows extra fields; the string index keeps this assignable. */
  [key: string]: string
}

/** Shape of a search payload we read from. */
interface SearchPayload {
  success?: boolean
  error?: unknown
  data?: Array<{ title?: unknown; url?: unknown; description?: unknown }>
  id?: unknown
}

/** Shape of a scrape payload we read from. */
...
/** Settings namespace id; branded at the type level only (runtime value is a plain string). */
const NAMESPACE = 'nativeWeb' as unknown as SettingsNamespace
 ...
 */
export function apply(ctx: Context, config: Partial<NativeWebConfig> = {}) {
  ...
  async function postJson<T>(path: string, body: unknown, timeoutMs: number, signal: AbortSignal | undefined): Promise<T> {
    const cfg = scope.get()
    ...
    try {
      ...
      const text = await response.text()
      let parsed: T & { success?: boolean; error?: unknown }
      try {
        parsed = JSON.parse(text) as T & { success?: boolean; error?: unknown }
      } catch {
  ...
  const searchTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      const resp = await postJson<SearchPayload>('/search', body, cfg.searchTimeoutMs, exec.signal)
      const results: SearchResult[] = (resp.data || []).map((d) => ({
  ...
  const scrapeTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      const resp = await postJson<ScrapePayload>('/scrape', body, cfg.scrapeTimeoutMs, exec.signal)
